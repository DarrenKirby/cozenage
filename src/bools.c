...
/* 'and' -> CELL_BOOLEAN|ANY - if any expression evaluates to #f, then #f is
...
Cell* builtin_and(const Lex* e, const Cell* a) {
    ...
    for (int i = 0; i < a->count; i++) {
        if (a->cell[i]->type == CELL_BOOLEAN && a->cell[i]->boolean_v == 0) {
...
/* 'or' -> CELL_BOOLEAN|ANY - the value of the first expression that evaluates
